...
class ChatAgent(BaseAgent):
    def manage_dialogue_and_cart(self, user_msg: str, chat_window: list = None, current_cart: list = None, last_session_cart: list = None) -> dict:
        ...
        - 劇本 B【精準替換 (replace)】：使用者改變心意，換地點或衝突條件（如：「那改去松山」、「公館附近呢」）。
        - 動作：🚨 地點唯一性原則！購物車內【永遠只能有一個地點】。只要使用者提到新地點，必須拔除舊地點，換成新地點！
        - 🛡️ 繼承鐵律：【絕對必須保留】原本不衝突的其他條件（例如：原本指定的時間、安靜等），絕對不可以把它們弄丟！
        【特殊鐵律】
        - 🏷️ updated_cart 裡面只能放【地點名稱】或【可用標籤清單】裡的字。
        - 🧹 確保 updated_cart 條件【絕對不重複】。
        - 🛡️ 記憶防護罩：執行劇本 A 與 B 時，絕對要保留原本的「時間」與「其他需求標籤」。
        - 📍 地點數量原則：通常情況下，購物車只會有一個地點。**【唯一例外】**：如果使用者「明確」說出要找「A跟B之間 / 兩者中間」的店，你才允許把【兩個地點】同時放進 updated_cart 和 keyword 中！
        - ⚠️ 【最高優先級鐵律】：如果是具體店名 (如 dine in cafe)，強制觸發劇本 C (clear)。
        ...
        【特殊鐵律】
        - 🏷️ updated_cart 裡面只能放【地點名稱】或【可用標籤清單】裡的字！不要放廢話！
        - 🧹 請確保 updated_cart 裡面的條件【絕對不重複】（例如不要出現兩個"安靜"）。
        - 🛡️ 記憶防護罩：在執行「劇本 A」與「劇本 B」時，除非使用者明確說不要了，否則【絕對要保留】原本購物車內的「時間條件」與「其他需求標籤」！
        - ⚠️ 最高優先級鐵律】：如果使用者說「是店名XXX」或提到具體店名（如 dine in cafe, always day one），【絕對、強制】觸發「劇本 C (clear)」，把購物車全部清空，且 keyword 與 updated_cart 裡面【只能留下店名】，絕不能夾帶舊條件！
